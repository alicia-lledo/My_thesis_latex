...
The technological advances experiences in sequencing and genotyping has allowed to implement association studies at a genome-wide scale. The gnome-wide association studies (GWAS) have benefit from the understanding of common (frequency ${>}$1\%) single base-pair changes know as single nucleotide polimorphisms (SNPs), in particular populations through whole genome sequencing (WGS) projects such as HapMap \parencite{The international HapMaP Consortium} project and the 1000 Genomes project \parencite{The 1000 Genomes}, which is actively expanding. Although SNPs may also include single nucleotide insertions or deletions, GWAS have focused in the association of single-nucleotide bi-allelic substitutions to a particular phenotype in a case-control design \parencite{Ku2010}. 

Due to the organisation of the genome into segments of strong linkage desiquilibrium (LD) where genetic 
variants are strongly correlated with each other, the genotyped SNPs are used a proxy for the disease causative variant. Those can be non-genotyped SNPs or other type of variation such as copy number variants (CNVs), also highly frequent in the genome \parencite{Hirschhorn2005, Ku2010}. This approach has greater power than the linkage studies when expecting many loci with small effects to influence  risk, as it is the case for complex diseases \parencite{Cui2010}.
